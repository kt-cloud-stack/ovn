 ...
 * EVPN Type-2 MAC+IP neighbor (into 'neighbors_v4'/'neighbors_v6').  ovn-northd
 * only populates a row when the datapath opted into advertising it (via the
 * 'ip'/'nat' redistribute tokens), so every present row is advertised here.
 * With 'fdb' set in 'mode' the MAC is additionally programmed into the FDB
 * ('fdb_neighbors').  Any of the target hmaps may be NULL when the
